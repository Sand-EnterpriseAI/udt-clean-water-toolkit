...
from cwageodjango.assets.models import WaterTank
from cwageodjango.utilities.models import DMA, Utility


class Command(BaseCommand):
    help = "Write Severn Trent Water Water Tank layer data to sql"
...
Large numbers of features will take a long time to save."""
        ...
        water_tank_layer = ds[layer_index]

        new_water_tanks = []
        for feature in water_tank_layer:
            gid = feature.get("tag")
            geom = feature.geom
            geom_4326 = feature.get("wkt_geom_4326")

            new_water_tank = WaterTank(
                tag=gid, geometry=geom.wkt, geometry_4326=geom_4326
            )
            new_water_tanks.append(new_water_tank)

            if len(new_water_tanks) == 100000:
                WaterTank.objects.bulk_create(new_water_tanks)
                new_water_tanks = []
        ...
        DMAThroughModel = WaterTank.dmas.through
        bulk_create_list = []
        for water_tank in WaterTank.objects.filter(dmas=None).only("id", "geometry"):
            wkt = water_tank.geometry.wkt
            ...
            bulk_create_list.extend(
                [
                    DMAThroughModel(watertank_id=water_tank.pk, dma_id=dma_id)
