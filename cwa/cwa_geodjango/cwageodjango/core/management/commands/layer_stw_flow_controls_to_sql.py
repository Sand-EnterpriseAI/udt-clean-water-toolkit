...
from cwageodjango.assets.models import FlowControl
from cwageodjango.utilities.models import DMA, Utility


class Command(BaseCommand):
    help = "Write Severn Trent Water Flow Control layer data to sql"
...
Large numbers of features will take a long time to save."""
        ...
        flow_control_layer = ds[layer_index]

        new_flow_controls = []
        for feature in flow_control_layer:
            gid = feature.get("tag")
            geom = feature.geom
            geom_4326 = feature.get("wkt_geom_4326")

            new_flow_control = FlowControl(
                tag=gid, geometry=geom.wkt, geometry_4326=geom_4326
            )
            new_flow_controls.append(new_flow_control)

            if len(new_flow_controls) == 100000:
                FlowControl.objects.bulk_create(new_flow_controls)
                new_flow_controls = []

        # save the last set of data as it will probably be less than 100000
        if new_flow_controls:
            FlowControl.objects.bulk_create(new_flow_controls)

        # get the utility
        utility = Utility.objects.get(name="severn_trent_water")

        DMAThroughModel = FlowControl.dmas.through
        bulk_create_list = []
        for flow_control in FlowControl.objects.filter(dmas=None).only(
            "id", "geometry"
        ):
            wkt = flow_control.geometry.wkt
            ...
            bulk_create_list.extend(
                [
                    DMAThroughModel(flowcontrol_id=flow_control.pk, dma_id=dma_id)
