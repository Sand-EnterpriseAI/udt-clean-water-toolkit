...
echo "Setting up the udt toolkit for staging."
...
docker compose -f ../../docker/docker-compose-postgis.yml -f ../../docker/docker-compose-neo4j.yml -f ../../docker/docker-compose-cwa-geodjango.yml -f ../../docker/docker-compose-cwa-geoalchemy.yml -f ../../docker/docker-compose-geoserver.yml -f ../../docker/docker-compose-neo4j-browser.yml -f ../../docker/docker-compose-api-drf.yml up -d --build
...
API_DRF_CONTAINER_ID=`docker ps | grep udtapidrf | grep api_drf_staging | awk '{ print $1 }'`


docker exec -it ${CWA_GEODORM_CONTAINER_ID} pip install -r requirements.txt
...
docker exec -it ${CWA_GEODORM_CONTAINER_ID} bash -c "cd /opt/udt/api/api_drf/ && pip install -r requirements.txt"

docker exec -it ${CWA_GEODORM_CONTAINER_ID} bash ln -s /opt/cwa/cwa_geodjango/cwa_geod/ /opt/api/api_drf/

docker exec -it ${API_DRF_CONTAINER_ID} pip3 install -r requirements.txt

../postgis/postgis_db_init.sh

echo
echo "cwa_geodjango app staging setup complete."
echo "cwa_geoalchemy app staging setup complete."
echo "api_drf app staging setup complete."
